...
class Classifier_RESNET:
    ...
    def fit(self, x_train, y_train, x_val, y_val, y_true_val, x_test, y_test, y_true_test):
        ...
        df_metrics = save_logs(self.output_directory, hist, y_pred_val, y_pred_val_proba, y_true_val, duration)
        df_metrics_test = save_logs(self.output_directory, hist, y_pred_test, y_pred_test_proba, y_true_test, duration)
        ...
        metrics_val.to_csv(self.output_directory + 'df_metrics_val.csv', index=False)
        metrics_test.to_csv(self.output_directory + 'df_metrics_test.csv', index=False)
    ...
    def aggregate_results(self, nb_classes):
        tprs = []
        mean_fpr = np.linspace(0, 1, 100)

        y_pred_test_proba = np.load(self.output_directory + f'y_pred_test.npy')
        y_true_test = np.load(self.output_directory + f'y_true_test.npy')
        y_pred_test = np.argmax(y_pred_test_proba, axis=-1)
