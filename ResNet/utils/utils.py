...
def save_logs_t_leNet(output_directory, hist, y_pred, y_true, duration):
    ...
    plot_epochs_metric(hist, output_directory + 'epochs_accuracy.png', metric='accuracy', name='Accuracy')



def save_logs(output_directory, hist, y_pred, y_pred_proba, y_true, duration, lr=True, y_true_val=None, y_pred_val=None):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(output_directory + 'history.csv', index=False)
    ...
    df_best_model.to_csv(output_directory + f'df_best_model.csv', index=False)
