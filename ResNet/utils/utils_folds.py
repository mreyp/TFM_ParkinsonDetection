...
def save_logs(output_directory, hist, y_pred, y_pred_proba, y_true, duration, fold_index, lr=True, y_true_val=None, y_pred_val=None):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(output_directory + 'history.csv', index=False)
...
def viz_plot(root_dir, df):
    ...
    # df_lengths['fcn']
    plt.savefig(root_dir + 'plot.pdf', dpi=300)
