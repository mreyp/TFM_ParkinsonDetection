### ------------ MODIFIED FILE: COMPARISON FOR SEVERAL GENERATED DATA IN BIGVGAN/INFERENCE.PY ------------ ###
...
def evaluate(gt_dir, synth_dir):
    ...
    gt_files = [file for file in os.listdir(gt_dir) if file.endswith('.wav')]
    synth_files = [file for file in os.listdir(synth_dir) if file.endswith('.wav')]
    ...
    with torch.no_grad():
        iterator = tqdm(gt_files, dynamic_ncols=True, desc=f'Evaluating {gt_dir} against {synth_dir}')
        
        for wavID in iterator:
            num_real += 1
            print(num_real)
            base_name = os.path.splitext(wavID)[0]

            # Inicializa la lista para archivos coincidentes
            matching_synth_files = []
            for f in synth_files:
                if base_name in os.path.splitext(f)[0]:
                    matching_synth_files.append(f)
                    if len(matching_synth_files) == 40:  # Verifica si ya se encontraron 40 archivos
                        break
        
            gt_path = os.path.join(gt_dir, wavID)
            y = load_wav(gt_path)
            y = y.to(device)
            y_long = y.unsqueeze(1)
            ...
            for synth_wav in matching_synth_files:
                num_comparisons += 1
                
                synth_path = os.path.join(synth_dir, synth_wav)
                y_g_hat = load_wav(synth_path)
                ...
                # MRSTFT calculation
                y_hat_long = y_g_hat.unsqueeze(1)
                #print('y_long', y_long.shape, 'y_hat_long', y_hat_long.shape)
                mrstft_tot += loss_mrstft(y_hat_long, y_long).item()
    ...
    return {
        'M-STFT': mrstft_tot / num_comparisons,
        'PESQ': pesq_tot / num_comparisons,
