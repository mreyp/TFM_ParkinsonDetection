...
class Model():
    def __init__(self, x, y, units, dropout, dropout2, filters, kernel_sizes, val_data):
        ...
        x, y = shuffle(x, y)
        self.x_mean = np.nanmean(x, axis=1, keepdims=True)
        self.x_std = np.nanstd(x, axis=1, keepdims=True)
        x = (x - self.x_mean) / (self.x_std + 1e-8)  # Evita división por cero
        ...
        x_val_mean = np.nanmean(x_val, axis=1, keepdims=True)
        x_val_std = np.nanstd(x_val, axis=1, keepdims=True)
        x_val = (x_val - x_val_mean) / (x_val_std + 1e-8)
    ...
    def fit(self, learning_rate, batch_size, epochs, verbose=True):
        ...
        #scheduler = StepLR(optimizer, 10, gamma=0.6)
    ...
    def predict(self, x_test):
        ...
        xmean = np.nanmean(x_test, axis=1, keepdims=True)
        xstd = np.nanstd(x_test, axis=1, keepdims=True)
        x_test = (x_test - xmean) / (xstd + 1e-8)
    ...
    def evaluate(self, y_pred, targets_test, class_names):
        ...
        loss = log_loss(y_pred, targets_test)

        print(f'Loss: {loss}, Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1 Score: {f1:.4f}, AUC: {auc}')
