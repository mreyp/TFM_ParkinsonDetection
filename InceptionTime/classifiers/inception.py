...
class Classifier_INCEPTION:
    ...
    def fit(self, x_train, y_train, x_val, y_val, y_true_val, plot_test_acc=False):
    ...
    def predict(self, x_t, y_true, return_df_metrics=True):
        start_time = time.time()
        model_path = self.output_directory + 'best_model.hdf5'
        model = keras.models.load_model(model_path)
        y_pred_proba = model.predict(x_t, batch_size=self.batch_size)
        if return_df_metrics:
            y_pred = np.argmax(y_pred_proba, axis=1)
            df_metrics = calculate_metrics(y_true, y_pred, y_pred_proba, 0.0)
            return df_metrics
        else:
