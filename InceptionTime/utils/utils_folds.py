...
def calculate_metrics(y_true, y_pred, y_pred_proba, duration, y_true_val=None, y_pred_val=None):
    res = pd.DataFrame(data=np.zeros((1, 6), dtype=np.float), index=[0],
                       columns=['accuracy','precision', 'recall','f1', 'AUC','duration'])
...
def plot_and_save_confusion_matrix(y_true, y_pred, path, class_names):
...
def transform_labels(y_train, y_val, y_test):
    ...
    # Concatena los conjuntos de entrenamiento, validación y prueba para ajustar
    y_combined = np.concatenate((y_train, y_val, y_test), axis=0)
    # Ajusta el codificador
    encoder.fit(y_combined)
    # Transforma a etiquetas mínimas cero y continuas
    new_y_combined = encoder.transform(y_combined)
    # Reasigna los conjuntos de entrenamiento, validación y prueba
    new_y_train = new_y_combined[:len(y_train)]
    new_y_val = new_y_combined[len(y_train):len(y_train) + len(y_val)]
    new_y_test = new_y_combined[-len(y_test):]
...
def generate_results_csv(output_file_name, root_dir, clfs):
    res = pd.DataFrame(data=np.zeros((0, 8), dtype=np.float), index=[],
                       columns=['classifier_name', 'archive_name', 'dataset_name', 'iteration',
                                'accuracy', 'precision', 'recall', 'duration'])
...
def plot_epochs_metric(hist, file_name, metric='loss', name='Loss'):
    plt.figure()
...
def save_logs(output_directory, hist, y_pred, y_pred_proba, y_true, duration, fold_index, 
              lr=True, plot_test_acc=True):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(output_directory + 'history.csv', index=False)

    df_metrics = calculate_metrics(y_true, y_pred, y_pred_proba, duration)
    ...
    df_best_model.to_csv(output_directory + f'df_best_model{fold_index}.csv', index=False)

    if plot_test_acc:
        # plot losses
        plot_epochs_metric(hist, output_directory + f'epochs_loss{fold_index}.png')
        plot_epochs_metric(hist, output_directory + f'epochs_accuracy{fold_index}.png', metric='accuracy', name='Accuracy')
